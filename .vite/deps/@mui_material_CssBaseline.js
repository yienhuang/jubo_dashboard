...
import {
  ...
  b as useDefaultProps$1,
  i as defaultTheme,
  r as identifier_default,
  y as DefaultPropsProvider$1,
  ...
  if (
    ...
    Object.entries(theme.colorSchemes).forEach(([key, scheme]) => {
      ...
      if (selector.startsWith('@'))
        colorSchemeStyles[selector] = { ':root': { colorScheme: scheme.palette?.mode } }
      else
...
var staticStyles = (theme) => {
  ...
  if (theme.colorSchemes)
    Object.entries(theme.colorSchemes).forEach(([key, scheme]) => {
      ...
      if (selector.startsWith('@'))
        baseStyles[selector] = {
          [`:root:not(:has(.${SELECTOR}))`]: { colorScheme: scheme.palette?.mode },
        }
      else
        baseStyles[selector.replace(/\s*&/, '')] = {
          [`&:not(:has(.${SELECTOR}))`]: { colorScheme: scheme.palette?.mode },
        }
    })
  return result
}
...
function CssBaseline(inProps) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_react.Fragment, {
    children: [
      isDynamicSupport &&
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(GlobalStyles, { enableColorScheme }),
      !isDynamicSupport &&
        !enableColorScheme &&
