    ...
    <ShareableBlock title="營運概況">
      ...
      >
        <Box className="flex flex-col gap-4">
          ...
          <Grid container spacing={2} sx={{ alignItems: 'stretch' }}>
            <Grid size={{ xs: 12, md: 6 }}>
              <OutlinedBlock title="出席率趨勢" subtitle="近 13 個月（25/05 ～ 26/05）">
                <TrendLineChart
                  months={attendanceTrend.months}
...
function BranchOperationsSection({ data, branchName, attendanceSeries, newCasesSeries }) {
  return (
...
function BranchTab({ branchName }) {
  ...
  const branchAttendanceSeries = useMemo(() => {
    const match = attendanceTrend.series.find((s) => s.name === branchName)
    return match ? [{ name: '出席率', color: PRIMARY, data: match.data }] : []
  }, [branchName])
  ...
  return (
    <Box className="flex flex-col gap-4">
      ...
      <BranchOperationsSection
        ...
        attendanceSeries={branchAttendanceSeries}
...
export default function DayCare() {
  ...
  const pageTitle = isValidBranch ? (BRANCH_FULL_NAME[branch] ?? branch) : '日照機構總覽'
