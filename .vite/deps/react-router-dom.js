  ...
  if (typeof route.lazy === 'object') {
    ...
    ;['middleware', 'loader', 'action'].forEach((key) => {
      ...
      if (typeof lazyFn === 'function' && instrumentations.length > 0) {
        ...
        if (instrumented)
          updates.lazy = Object.assign(updates.lazy || {}, { [key]: instrumented })
      }
    })
  }
    ...
    let fetcherResultsPromise = Promise.all(
      fetchersToLoad.map(async (f) => {
        if (f.matches && f.match && f.request && f.controller) {
          let result = (
            await callDataStrategy(f.request, f.path, f.matches, scopedContext, f.key)
          )[f.match.route.id]
          return { [f.key]: result }
        } else
    ...
    if (init.patchRoutesOnNavigation) {
      ...
      if (!matches)
        return {
          active: true,
          matches:
            matchRoutesImpl(routesToUse, pathname, basename, true, activeBranches) || [],
        }
              ...
              let staticContext = {
                ...
                errors: {
                  [skipLoaderErrorBubbling
                    ...
                    : findNearestBoundary(
                        matches,
                        matches.find((m) => m.route.id === routeId || m.route.loader)
                          ?.route.id || routeId,
                      ).route.id]: error,
                },
  ...
  let matches =
    dataRouterOpts && dataRouterOpts.state.matches.length
      ? dataRouterOpts.state.matches.map((m) =>
          Object.assign(m, { route: dataRouterOpts.manifest[m.route.id] || m.route }),
        )
...
function DefaultErrorComponent() {
  ...
  return /* @__PURE__ */ import_react.createElement(
    ...
    stack
      ? /* @__PURE__ */ import_react.createElement('pre', { style: preStyles }, stack)
      : null,
    devInfo,
  )
}
...
function useScrollRestoration({ getKey, storageKey } = {}) {
  ...
  let { restoreScrollPosition, preventScrollReset } =
    useDataRouterState2('useScrollRestoration')
  let { basename } = import_react.useContext(NavigationContext)
  let location = useLocation()
  let matches = useMatches()
